...
# Output prefix for the simplified/raw-feature version so files do not overwrite
# the original weighted/composite-feature script outputs.
OUTPUT_PREFIX = "trained_hr_model_simplified_raw_features"
...
FEATURE_COLUMNS = [
    ...
    # Pitcher raw vulnerability / suppression
    "pitcher_pa_prior",
    ...
    # Pitch-type matchup; retained because it adds genuinely matchup-specific information.
    "pitch_fit_score_prior",
    "pitch_fit_coverage_prior",

    # Direct BvP/contact-history signals only; the hand-built composite is kept for display,
    # but not used as a model input in this simplified version.
...
def add_macro_board_columns(board: pd.DataFrame) -> pd.DataFrame:
    ...
    # Sample-aware direct BvP component. Small samples are heavily discounted and
    # only reach full weight at 15 prior PA.
    bvp_confidence = (_safe_series(out, "matchup_pa_prior") / 15.0).clip(0.0, 1.0)
    bvp_history_raw = (
    ...
    bvp_history_confidence_adjusted_raw = bvp_history_raw * bvp_confidence

    # Projected matchup quality, not merely historical BvP. The four inputs are
    # kept on their natural raw scales here and converted to slate percentiles below.
    batter_split_component_raw = (
        0.40 * _safe_series(out, "batter_hr_rate_vs_hand_prior")
        + 0.25 * _safe_series(out, "batter_barrel_rate_vs_hand_prior")
        + 0.15 * _safe_series(out, "batter_hard_hit_rate_vs_hand_prior")
        + 0.20 * (_safe_series(out, "batter_avg_ev_vs_hand_prior") / 100.0)
    )
    pitcher_split_component_raw = (
        0.40 * _safe_series(out, "pitcher_hr_rate_vs_batter_hand_prior")
        + 0.25 * _safe_series(out, "pitcher_barrel_rate_vs_batter_hand_prior")
        + 0.15 * _safe_series(out, "pitcher_hard_hit_rate_vs_batter_hand_prior")
        + 0.20 * (_safe_series(out, "pitcher_avg_ev_vs_batter_hand_prior") / 100.0)
    )
    matchup_advantage_raw = (
        0.40 * _safe_series(out, "pitch_fit_score_prior")
        + 0.25 * batter_split_component_raw
        + 0.20 * pitcher_split_component_raw
        + 0.15 * bvp_history_confidence_adjusted_raw
    )
    ...
    raw_map = {
        ...
        "matchup_advantage": matchup_advantage_raw,
    ...
    # Supporting matchup detail for CSV/report-card use.
    out["bvp_confidence_pct"] = (bvp_confidence * 100).round(1)
    out["bvp_history_confidence_adjusted_raw"] = bvp_history_confidence_adjusted_raw
    out["matchup_pitch_fit_component"] = (
        _safe_series(out, "pitch_fit_score_prior").rank(method="average", pct=True).fillna(0) * 100
    ).round(1)
    out["matchup_batter_split_component"] = (
        batter_split_component_raw.rank(method="average", pct=True).fillna(0) * 100
    ).round(1)
    out["matchup_pitcher_split_component"] = (
        pitcher_split_component_raw.rank(method="average", pct=True).fillna(0) * 100
    ).round(1)
    out["matchup_bvp_component"] = (
        bvp_history_confidence_adjusted_raw.rank(method="average", pct=True).fillna(0) * 100
    ).round(1)
...
def print_feature_importance(
    ...
    max_rows: int = FEATURE_AUDIT_VALID_SAMPLE_ROWS,
    n_repeats: int = 5,
    top_n: int = 20,
) -> pd.DataFrame:
    """
    Print and save a compact model-agnostic feature importance report.

    HistGradientBoostingClassifier does not expose native feature_importances_,
    so this uses permutation importance on a holdout/reference sample. The value
    means: how much validation log loss gets worse when that feature is shuffled.
    Higher = the model relied on that feature more for out-of-sample prediction.

    Delta columns compare the current rank to the previously saved report.
      + means the feature moved up in importance.
      - means the feature moved down.
    """
    feature_cols = get_model_feature_columns()
    available = [c for c in feature_cols if c in reference_df.columns]
    report_path = f"{OUTPUT_PREFIX}_feature_importance_report_{label}.csv"

    previous = pd.DataFrame()
    if os.path.exists(report_path):
        try:
            previous = pd.read_csv(report_path)
            if "feature" in previous.columns:
                if "rank" not in previous.columns:
                    previous = previous.copy()
                    previous["rank"] = np.arange(1, len(previous) + 1)
                previous = previous[["feature", "rank"]].rename(columns={"rank": "previous_rank"})
        except Exception:
            previous = pd.DataFrame()

    if reference_df.empty or not available or "home_run_game" not in reference_df.columns:
        print("\nNo reference rows available for feature importance report.")
        return pd.DataFrame(columns=[
            "rank", "feature", "importance_mean", "importance_std", "importance_pct",
            "previous_rank", "rank_delta", "target_corr_abs", "nonzero_rate", "mean_value", "std_value"
        ])

    ref = reference_df.copy()
    if len(ref) > max_rows:
        ref = ref.sample(max_rows, random_state=FEATURE_AUDIT_RANDOM_STATE).copy()

    x_ref = ref[available].apply(pd.to_numeric, errors="coerce").fillna(0)
    y_ref = ref["home_run_game"].astype(int)
    ...
    try:
        ...
        print(f"\n⚠️ Feature importance report failed: {e}")
        return pd.DataFrame(columns=[
            "rank", "feature", "importance_mean", "importance_std", "importance_pct",
            "previous_rank", "rank_delta", "target_corr_abs", "nonzero_rate", "mean_value", "std_value"
        ])

    target_corr = _safe_abs_corr_with_target(ref, available, "home_run_game")
    report = report.merge(target_corr, on="feature", how="left")
    report["nonzero_rate"] = [(x_ref[c] != 0).mean() for c in available]
    report["mean_value"] = [x_ref[c].mean() for c in available]
    report["std_value"] = [x_ref[c].std() for c in available]

    # Negative permutation importance means shuffling helped slightly on this sample;
    # treat it as zero for percentage-of-positive-importance display.
    report["positive_importance"] = report["importance_mean"].clip(lower=0)
    total_positive = report["positive_importance"].sum()
    report["importance_pct"] = np.where(
        total_positive > 0,
        report["positive_importance"] / total_positive * 100.0,
        0.0,
    )

    report = report.sort_values(
        ["importance_mean", "target_corr_abs"],
        ascending=False,
    ).reset_index(drop=True)
    report.insert(0, "rank", np.arange(1, len(report) + 1))

    if not previous.empty and "feature" in previous.columns:
        report = report.merge(previous, on="feature", how="left")
    else:
        report["previous_rank"] = np.nan

    report["rank_delta"] = report["previous_rank"] - report["rank"]

    output_cols = [
        "rank", "feature", "importance_mean", "importance_std", "importance_pct",
        "previous_rank", "rank_delta", "target_corr_abs", "nonzero_rate", "mean_value", "std_value"
    ]
    report_out = report[output_cols].copy()

    display = report_out.head(top_n).copy()
    display["importance_mean"] = display["importance_mean"].round(6)
    display["importance_pct"] = display["importance_pct"].round(1)
    display["previous_rank"] = display["previous_rank"].apply(lambda x: "—" if pd.isna(x) else int(x))
    display["rank_delta"] = display["rank_delta"].apply(
        lambda x: "—" if pd.isna(x) else (f"+{int(x)}" if int(x) > 0 else str(int(x)))
    )

    print(f"\n=== TOP {top_n} FEATURE IMPORTANCE ({label.upper()} PERMUTATION LOG LOSS) ===")
    print("Interpretation: higher importance = shuffling that feature hurt holdout log loss more; Δ rank compares to the previous run.")
    print(display[["rank", "feature", "importance_mean", "importance_pct", "previous_rank", "rank_delta"]].to_string(index=False))

    report_out.to_csv(report_path, index=False)
    print(f"\nSaved: {report_path}")

    return report_out


def _format_shap_factors(pairs: list[tuple[str, float]], max_items: int = 5) -> str:
    if not pairs:
        return ""
    out = []
    for feature, val in pairs[:max_items]:
        sign = "+" if val >= 0 else ""
        out.append(f"{feature} {sign}{val:.4f}")
    return "; ".join(out)


def print_top_pick_shap_explanations(
    model,
    board: pd.DataFrame,
    reference_df: pd.DataFrame,
    top_n: int = 10,
    ...
    """
    Print SHAP explanations for the top daily picks.

    Primary path uses the shap package. If SHAP is unavailable or does not support
    this sklearn estimator in the local environment, a conservative fallback uses
    one-feature-at-a-time replacement with validation medians to approximate local
    directional impact on raw probability.
    """
    feature_cols = get_model_feature_columns()
    available = [c for c in feature_cols if c in board.columns]
    if board.empty or not available:
        print("\nNo board rows available for SHAP explanations.")
        return pd.DataFrame()
    ...
    if ranked.empty:
        print("\nNo ranked rows available for SHAP explanations.")
        return pd.DataFrame()

    x_top = ranked[available].apply(pd.to_numeric, errors="coerce").fillna(0)
    bg = reference_df.copy()
    if not bg.empty:
        bg_available = [c for c in available if c in bg.columns]
        bg = bg[bg_available].apply(pd.to_numeric, errors="coerce").fillna(0)
        if len(bg) > 500:
            bg = bg.sample(500, random_state=FEATURE_AUDIT_RANDOM_STATE)
        # Ensure same feature order/columns.
        for c in available:
    ...
    try:
        ...
        explanation = explainer(x_top)
        values = explanation.values
        if isinstance(values, list):
            values = values[-1]
        values = np.asarray(values)
        # Some explainers return shape (rows, features, classes). Use positive class.
        if values.ndim == 3:
            values = values[:, :, -1]
        if values.shape[0] != len(x_top) or values.shape[1] != len(available):
            raise ValueError(f"Unexpected SHAP shape {values.shape}; expected ({len(x_top)}, {len(available)})")
    except Exception as e:
        method = "median_probability_delta_fallback"
        print(f"\n⚠️ SHAP calculation failed; using median-replacement fallback explanations instead: {e}")
        medians = bg.median(numeric_only=True).reindex(available).fillna(0)
        base_probs = predict_raw(model, ranked)
        values = np.zeros((len(ranked), len(available)), dtype=float)
        for j, feature in enumerate(available):
            temp = ranked.copy()
            temp[feature] = medians[feature]
            new_probs = predict_raw(model, temp)
            values[:, j] = base_probs - new_probs

    rows = []
    for i, (_, row) in enumerate(ranked.iterrows()):
        contribs = [(feature, float(values[i, j])) for j, feature in enumerate(available)]
        positive = sorted([p for p in contribs if p[1] > 0], key=lambda x: x[1], reverse=True)
        negative = sorted([p for p in contribs if p[1] < 0], key=lambda x: x[1])
        rows.append({
            "ranking": int(row.get("ranking", i + 1)) if pd.notna(row.get("ranking", i + 1)) else i + 1,
            "batter_name_hand": row.get("batter_name_hand", row.get("batter_name", "")),
            "batting_team": row.get("batting_team", ""),
            "pitcher_name_hand": row.get("pitcher_name_hand", row.get("pitcher_name", "")),
            "final_hr_probability": row.get("final_hr_probability", np.nan),
            "top_positive_factors": _format_shap_factors(positive, top_features),
            "top_negative_factors": _format_shap_factors(negative, top_features),
            "explanation_method": method,
        })

    explain_df = pd.DataFrame(rows)
    print(f"\n=== TOP {top_n} PICK SHAP EXPLANATIONS ===")
    print("Positive factors pushed the model probability up; negative factors pushed it down. Values are model-scale contributions.")
    print(explain_df[[
        "ranking", "batter_name_hand", "pitcher_name_hand", "final_hr_probability",
        "top_positive_factors", "top_negative_factors"
    ]].to_string(index=False))

    out_path = f"{OUTPUT_PREFIX}_top_pick_shap_explanations_{TARGET_DATE}.csv"
...
def run_feature_audit_and_select(
    ...
    corr_long.to_csv(f"{OUTPUT_PREFIX}_feature_redundancy_pairs.csv", index=False)
    ...
    audit.to_csv(f"{OUTPUT_PREFIX}_feature_audit.csv", index=False)
    pd.DataFrame(kept_preference).to_csv(f"{OUTPUT_PREFIX}_feature_correlation_pruning.csv", index=False)
    pd.DataFrame({"feature": selected}).to_csv(f"{OUTPUT_PREFIX}_selected_features.csv", index=False)

    print(f"Original features: {len(available)}")
    print(f"Selected features: {len(selected)}")
    print(f"Dropped features: {len(available) - len(selected)}")
    print("Saved:")
    print(f" - {OUTPUT_PREFIX}_feature_audit.csv")
    print(f" - {OUTPUT_PREFIX}_feature_redundancy_pairs.csv")
    print(f" - {OUTPUT_PREFIX}_feature_correlation_pruning.csv")
    print(f" - {OUTPUT_PREFIX}_selected_features.csv")
...
def run_backtest(model, calibrator, test_df: pd.DataFrame) -> None:
    ...
    scored.to_csv(f"{OUTPUT_PREFIX}_scored_test_rows.csv", index=False)
    summary_df.to_csv(f"{OUTPUT_PREFIX}_backtest_summary.csv", index=False)

    print("\nSaved:")
    print(f" - {OUTPUT_PREFIX}_scored_test_rows.csv")
    print(f" - {OUTPUT_PREFIX}_backtest_summary.csv")
...
def run_forward_hr_check(board: pd.DataFrame, target_date: str) -> pd.DataFrame:
    ...
    graded_path = f"{OUTPUT_PREFIX}_board_graded_{target_date}.csv"
    summary_path = f"{OUTPUT_PREFIX}_board_hr_check_summary_{target_date}.csv"
...
# =========================================================
def main():
    print("\n=== SIMPLIFIED RAW-FEATURE HR MODEL VERSION - NO UNIQUE PITCHERS FACED (NO DK) ===")
    ...
    print_feature_importance(model, valid_df, label="validation")
